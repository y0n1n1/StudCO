...
const Calendar: React.FC = () => {
  ...
  return (
    ...
    <div className="w-full h-5/6 bg-white flex flex-col sm:flex-row p-4 pt-0 px-8 border-t ">
      ...
      {/* Calendar Grid */}
      <div className="overflow-y-scroll h-75svh  flex-1 p-4 pt-0 pl-0">
              ...
              <div key={monthIndex} id={`month-${monthIndex}`} className="contents">
                {days.map((day, idx) => {
                  ...
                  return (
                    <div
                      key={`${month}-${idx}`}
                      className={`h-36 flex border-t border-r border-stone-300 flex-col items-end justify-start p-2 pr-0 md:pr-2 hover:bg-stone-50 cursor-pointer ${tileBgColor}`}
  ...
  return (
    <Dialog key={idx}>
      <DialogTrigger>
        <div className={`${event.color} text-white text-xs sm:text-sml md:text-base lg:text-2xl rounded px-2 py-1 my-1  text-wrap justify-start`}>
        <span
          className=""
